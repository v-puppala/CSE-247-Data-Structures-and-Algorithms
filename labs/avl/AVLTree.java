...
public class AVLTree<T extends Comparable<T>> {
	...
	private boolean existsHelper(T value, TreeNode<T> root) {
		if (root == null) { // not found
	    	...
	    	int xcomparison = value.compareTo(root.value);
		
	    	if (xcomparison == 0) { // found
	    		return true;
	    	} else if (xcomparison < 0) { // still looking - go left
	...
	//
	// EFFECT: Set the of t root's height field to the updated value
	//
	...
	//if (root.right==null||root.left.right==null) {
	//		return root;
	//	}
		TreeNode<T> A=root;
