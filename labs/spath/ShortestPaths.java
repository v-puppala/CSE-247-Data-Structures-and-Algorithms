...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	//   - get the handle to the vertex from the heapa
    	//   - extract the vertex + distance object from the handled
    	...
    	for(Edge i: vert.vertex.edgesFrom()) {
    		if(vert.distance+weights.get(i)<(handles.get(i.to)).getValue().distance) {
    			int uDist=handles.get(i.to).getValue().distance;
    			uDist=vert.distance+weights.get(i);
    			handles.get(i.to).decrease(new VertexAndDist(handles.get(i.to).getValue().vertex,uDist));
    			parentEdges.put(handles.get(i.to).getValue().vertex, i);
    		}	
    		//parentedge.endVertex
    		//}
    	}
    		
    			
    			//weights.get( whatever edge is
