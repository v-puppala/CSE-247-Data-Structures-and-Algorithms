...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	//insert adds the element to end of array and decrease swaps
		//heapify swaps in opposite direction of decrease
		//call heapify after extractMin and put last element on top and then call heapify to swap downwards
		//for extractMin, you're supposed to add last element on top, first get value 
			//be sure to call heapify at end of extractMin
		//heapify is called recursively
		//Ask if the paring is insert calls decrease and extractMin calls heapify?
		//Ask why decrease is public
		//Decrease needs to be public because the decreaser method of the decreaser class works to change value of a decreaser 
		//Decreasefddddd
	/**
	 ...
	 *   decrease(int loc) when necessary.--reference is h
	 */
	
	//call decrease from insert method
	public Decreaser<T> insert(T thing) {
		...
		Decreaser<T> ans = new Decreaser<T>(thing, this, ++size);//size +1
		ticker.tick();
		this.array[size]=ans;//index starts at 1 not 0
		this.array[size].loc=size;

		if (ans.loc>1&&ans.loc<=size&&ans.loc/2>=1) {
		decrease(ans.loc);
		
				
	//how do i add ans to the array--what is the syntax?
...
//
	 ...
	 *     decreased in value
	 */
	void decrease(int loc) {//decrease does the recursive swap
		
		if(loc>=0&&loc<=size&&loc/2>=1) {
	...
	public T extractMin() {
		...
		//	if(this.array[z].getValue()==null) {
				// maxValue=this.array[z-1];
			//	for(int k=z;z<=size;z++) {
			//	Decreaser<T> filler=this.array[k];
			//	}
			//}
	...
	private void heapify(int where) {
		if(where>size||where*2>size) {
			return;
		}
		else if (2*where+1<=size) {
		if(this.array[2*where].getValue().compareTo(this.array[2*where+1].getValue())!=0) {
			if(this.array[2*where].getValue().compareTo(this.array[2*where+1].getValue())<0) {
				if(this.array[2*where].getValue().compareTo(this.array[where].getValue())<0){
					...
					ticker.tick();
					this.array[where]=this.array[2*where];
					this.array[where].loc=where;
					this.array[2*where]=temp;
					this.array[2*where].loc=2*where;
					//this.array[where].loc=2*where;
					//this.array[where]=this.array[2*where];
					//this.array[2*where].loc=where;
					//this.array[2*where]=temp;
					ticker.tick();
			...
			else if(this.array[2*where].getValue().compareTo(this.array[2*where+1].getValue())>0) {
				if(this.array[2*where+1].getValue().compareTo(this.array[where].getValue())<0){
					Decreaser<T> temp= this.array[where];
					ticker.tick();//
					this.array[where]=this.array[2*where+1];
					this.array[where].loc=where;
					this.array[2*where+1]=temp;
					this.array[2*where+1].loc=2*where+1;
					ticker.tick();
		...
		else if(this.array[2*where].getValue().compareTo(this.array[2*where+1].getValue())==0 && this.array[where].getValue().compareTo(this.array[2*where].getValue())>0) {
			...
			if(this.array[where].getValue().compareTo(this.array[2*where].getValue())>0) {
				Decreaser<T> temp= this.array[where];
